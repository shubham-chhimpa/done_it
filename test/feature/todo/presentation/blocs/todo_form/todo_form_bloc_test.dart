...
void main() {
  ...
  group("TodoFormAddEvent", () {
    ...
    test("should emit [TodoFormLoadingState,TodoFormAddSuccessState] when data is added successfully", () async {
      // arrange
        when(mockAddTodo(any)).thenAnswer((realInvocation) async => Right(tTodo));
        // assert later
        final expected = [
        ...
        expectLater(todoFormBloc, emitsInOrder(expected));
        // act
        todoFormBloc.add(TodoFormAddEvent());

      });

    test("should emit [TodoFormLoadingState,TodoFormAddFailState] when data failed to add", () async {
      // arrange
      when(mockAddTodo(any)).thenAnswer((realInvocation) async => Left(DataBaseFailure()));
      // assert later
      final expected = [
        TodoFormLoadingState(),
